...
function getHtmlReport(type){
	...
	var docTypeId=$('#doctype').find(":selected").val();
	   	...
	   	var param="doctypeId="+docTypeId+"&fromDate="+datef+"&toDate="+dates;
	   	if(docTypeId!=''){
	   		$("#loadingImage").show();
	...
	$.ajax({
		...
		url : "<%=restURL%>/role/getDocumentTypeDetailReport?"+param,
		dataType : 'json',
		timeout : 100000,
	  //data:JSON.stringify({'email':email,'phone':mnumber,'ccode':'+91','userId':userid,'not':'nosms'}),
		success : function(recordsdata) {
			...
			if(recordsdata.key=="200"){
				var totalrecords='';
				 totalrecords=recordsdata.documenttype[0].uploadSource;
		 			...
		 			row=row+'<td  width="20%">LF User</td>';
				        ...
				        buttons: [
		 		 		...
		 		 		$.each(recordsdata.documenttype, function(i, obj) {
		 		 	 		...
		 		 	 		out.push( [obj.actualFileName,obj.propertyName,obj.documentType,obj.version,obj.createDate,name]);
		 		 	 	});
		...
		error : function(e) {
			...
			$("#result").text("Please Select Document Type");	
		}		
}
...
$(function() {
	setHeader('documentTypesDetailReports.jsp');
	$("#loadingImage").hide();
...
function initial_load()
	{
	setHeaderText('Document Type (Detailed) Report','Reporting & Analysis','dashboard.jsp?reports=true');
...
//-->
	...
	<section class="upload-document">
        <div class="container-fluid">
            ...
            <br>
                <h4 style="color: #0a68b4;">Document Type (Detailed) Report</h4>
				<hr>
				<div>This Report will provide details on all Records uploaded to a specific Document Type.<br>
 &nbsp  The Fields include: (File Name, iREMS#, Document Type,Version Total, Date Uploaded,Record Originator )</div><br>
                    ...
                    <div class="form-group">
							<div class="row">
								...
								<label class="control-label">Document Type :</label>
									<select id="doctype" style="height: 25px; width: 175px;    float: right;">
