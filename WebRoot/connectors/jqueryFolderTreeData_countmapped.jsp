...
<%@ page
	...
	<jsp:useBean id="PathStringsToTreeStructure" scope="request" class="com.peniel.utilities.PathStringsToTreeStructure" />
<%
 ...
 if(!StringUtils.isEmpty(propertyId)){
    ...
    //   }
        ...
        for(int j=0; j<pathArray.size(); j++)
	    {
 		...
 		String [] pathStringArray=null;
 	 if(!pathArray.isEmpty()){
 		 if(pathStringArray ==null || pathStringArray.length==0){
 			pathStringArray= pathArray.toArray(new String[pathArray.size()]);
 			output=PathStringsToTreeStructure.getFolderCounts(pathStringArray);
 		 }
 	 }
 	 ...
 	 /*********************new change*****************/
		...
		for (String file : files) {
		    ...
		    if(output.containsKey(dir+file)){
		    	count=output.get(dir+file);
		    }
		    if (map.containsKey(file)) {
		    	...
		    	if(countMap != null && countMap.containsKey(file)){
		    	 ...
		    	 dataLen= countMap.get(file)+ count;//map.get(file);
		    	}else
		    	{
		    		
		    		dataLen=count;//map.get(file);
		    	}
