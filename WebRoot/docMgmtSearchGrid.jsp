...
<% 
 ...
 // String contextLoginId = request.getParameter("contextLoginId").toString();
  ...
  /*HashMap companyHashMap = companyInformation.getcompanyInformation(request.getAttribute("comid").toString());
  String indexType = indexTypeBean.getIndexType(companyHashMap.get("defaultIndex").toString(),request.getAttribute("comid").toString());*/
...
function onInvokeAction(id,action) {
	...
	if(document.getElementById("indexval") != null){
		if(document.getElementById("indexval").value!= null ){
			...
			propertyName = document.getElementById("propertynameClassic").value;
			...
			if(fView == "fView")
			{
				...
				queryString += ""+"&indexvar="+indexvar+"&"+"docvar="+docvar+"&"+"foldervar="+foldervar+"&"+"indexval="+
								   indexval+"&"+"docStatus="+docStatus + "&"+"propertytype="+propertyType+ "&"+"propertyname="+propertyName;				
			}
	...
	var parameterString = createParameterStringForLimit(id);
    $.post("DocMgmtSearchGrid1?"+queryString+"&"+ parameterString, function(data) {
...
{ 
	if(fView == "fView")
    {	
		$('#fileTreeDemo_4FView').fileTree({ root: rootDir,expandedFolders: s , script: 'connectors/browseFolderFiles.jsp', folderEvent: 'click', expandSpeed: 750, collapseSpeed: 750, expandEasing: 'easeOutBounce', collapseEasing: 'easeOutBounce', loadMessage: 'One moment...' }, function(file) { 
	});
	}
	else if(fView == "advView")
	{
	...
	$('#fileTreeDemo_4').fileTree({ root: rootDir,expandedFolders: s , script: 'connectors/searchResult.jsp', folderEvent: 'click', expandSpeed: 750, collapseSpeed: 750, expandEasing: 'easeOutBounce', collapseEasing: 'easeOutBounce', loadMessage: 'One moment...' }, function(file) { 
	});
	}
}
    ...
    var comid = '<%= cid %>';
           ...
           $.ajax({
            ...
            success: function (data) {
             ...
             getPropertyNameValues('propertyname','propertynameClassic');
   ...
   if(fView != "fView" && fView != "advView") 
	{
	 	...
	 	document.getElementById('propertytypeClassic').value ="";
	 		document.getElementById('propertynameClassic').value ="";	
...
function validateForm()
{
    ...
    var propertyValue = document.getElementById("propertytype");
     var propertyNameValue = document.getElementById("propertyname");    
	...
	if(fView == "fView" && document.getElementById('folderNameFView').value =="") 
	{
		...
		return false;		
			...
			&& document.getElementById('propertyname').value ==""
			&& docTypeValue == "All")
			/* && propertyValue == "Any" && propertyNameValue == "Any"  */
	{
	    jAlert("Please select at least one search criteria.","Select Criteria");
	    return false;
	}
	else if(fView != "fView" && fView != "advView" && document.getElementById('indexval').value =="" && document.getElementById('propertytypeClassic').value =="" && document.getElementById('propertynameClassic').value ==""){
...
function getfiles() {
	...
	//alert(queryString);
			if(xmlhttp) { 
		    xmlhttp.open("POST","DocMgmtSearchGrid1",true); 
...
function getpdf1(id,docType,indexType,indexVal,folderVal,status,docTypeId,version,indexId,docId)
{
	...
	{
		var Text = "./DisplayText?id="+id+"&userid="+user+"&comid="+com+"&sessionid="+session+"&userName="+userName+"&keywordSearch="+keywordSearch;
	    var popout= window.open(Text,'_blank', 'toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,left=15000, top=15000, width=2, height=2, visible=none', ''); 
...
window.setTimeout(function(){
    popout.close();
	  ...
	  if(xmlhttp2) { 
			   xmlhttp2.open("POST", "./DisplayPdf?id="+id+"&userid="+user+"&comid="+com+"&sessionid="+session+"&userName="+userName, true);
...
function getDocumentData(id)
{
	...
	var ext = id.split('.').pop().toLowerCase();
	if(ext == "doc" || ext == "docx" || ext == "xlsx" || ext == "xls" || ext == "jpg" || ext == "jpeg")
	{
		var Text = "./DisplayText?id="+id+"&userid="+user+"&comid="+com+"&sessionid="+session+"&userName="+userName+"&keywordSearch="+keywordSearch;
	 var popout= window.open(Text,'_blank', 'toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,left=15000, top=15000, width=50, height=50, visible=none', ''); 
									...
									<tbody>
										<tr>
											...
											<td><!-- 	<select class="selectText" style="overflow:auto;width:250px;"  id="propertytype" name="propertytype">
													...
													<input id="propertyname" name="propertyname"  type="text" size=41 style="box-shadow: 0px 1px 4px 0px rgba(0, 0, 0, 0.3);"  >											
											</td>
											<td></td>
										</tr>
										<tr>
...
<div id="content2">			
	<div id = "moreRows">				
		<div class="card" style="width: 95%; padding-left: 0px;">
			<div class="card-body card-padding" style="background-color:#EDECEC">
								<table class="table" cellspacing="0">
									<tbody>
										...
										<tr>
											<td>Property Name</td>
											<td>	
													<input id="propertynameClassic" name="propertynameClassic"  type="text" size=49 style="box-shadow: 0px 1px 4px 0px rgba(0, 0, 0, 0.3);"  >											
											</td>
											<td></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td><input id="indexval" name="indexval" type="text" size=49 style="box-shadow: 0px 1px 4px 0px rgba(0, 0, 0, 0.3);" ></td>
										</tr>
				...
				<table width= "96%" align="center" border=0 style="margin-right: 30px;">
						...
						<form name='docSearch' id='docSearch' action="DocMgmtSearchGrid1" >	
								...
								<table class="table" cellspacing="0">
									<tbody>
										...
										<tr>
											...
											<input id="submitFolder" name="submitFolder" class="btn btn-success waves-effect waves-button waves-float" style="margin-right: 5px;"  type="button" onClick="javascript:getfiles()" value="Submit">
											 <button  id="clear" name="clear" class="btn btn-warning waves-effect waves-button waves-float" style="margin-right: 15px;" type="button" onClick="javascript:clearFields()">
                									<i class="zmdi zmdi-format-clear"></i> Clear
            								</button>
    	...
    	<script type="text/javascript" src="js/getPropertyNameValue.js"></script>
