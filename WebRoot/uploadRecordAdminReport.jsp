...
function displayData(data) {
  	...
  	$.each(data.uploadUsersReport, function(i, obj) {
...
function getHtmlReport(type){
	...
	if(user){
		...
		$("#result1").text("User Name Cannot be Blank");
		return false;
	}
if(month||year){
	if(year){
		...
		if(fromDate!='Invalid Date'){
			if(toDate!='Invalid Date'){
		    	...
		    	var dates=toDate.getMonth()+1+"-"+toDate.getDate()+"-"+toDate.getFullYear();
		    	 displaydate="From "+datef+" to "+dates;
			}else{
		...
		return false;
	   	...
	   	var param="startDate="+datef+"&endDate="+dates+"&year="+year+"&month="+month+"&user="+user;
	$("#loadingImage").show();

	$.ajax({
		type : "GET",
		contentType : "application/json",
		url : "<%=restURL%>/role/getUploadRecordAdminReport?"+param,
		dataType : 'json',
		timeout : 100000,
	  //data:JSON.stringify({'email':email,'phone':mnumber,'ccode':'+91','userId':userid,'not':'nosms'}),
		success : function(data) {
			...
			if(data.key=="200"){
				  ...
				  if(data.uploadUsersReport!=null){
					  ...
					  dates=year;
					   displaydate=datef+","+dates;
				   		...
				   		$("#myTable").dataTable({
					        ...
					        buttons: [
					        	{ extend : 'excelHtml5',
					        		title:"User Upload Record Report",
					        		text:"EXCEL REPORT",
					        		filename:"User Upload Record Report",
					        		messageTop: displaydate
					        	},{ extend : 'pdfHtml5',
					        		title:"User Upload Record Report",
					        		text:"PDF REPORT",
					        		filename:"User Upload Record Report",
...
function initial_load()
	...
	setHeader('uploadRecordAdminReport.jsp');
	setHeaderText('User Upload Record','Reporting & Analysis','dashboard.jsp?reports=true');
	...
	function checkUsername(){
		...
		if (!re.test($('#username').val())) {
			$("#result1").text("Please enter a valid User Name");
		    return false;
		}
		return true;
	}
...
//-->
	...
	<section class="upload-document">
        <div class="container-fluid">
            ...
            <br>
                <h4 style="color: #0a68b4;">User Upload Record Report</h4>
				<hr>
				<div>
				This Report provides a list of all records uploaded in the date range provided, by User.<br>
The Fields include: (User, File Name, iREMS#, Document Type, Upload Date)
</div><br>
							
	<div >
				<span id="result1" style="color:red;margin-left:15px"></span></div>			
<div class="rowSetting" style="padding-top: 20px;">			
<div class="fg-line">
<input id="username" name="username"  maxlength="50" style="width:190px;height:30px;margin-left:15px;" placeholder="H# (Hxxxxx) *" type="text"  onblur="checkUsername();">
                             </div>
                             </div> 
                             <br>      <br>       
<div class="upload-dates col-sm-12">
				...
				</div>                <div class="upload-dates col-sm-12">
                 ...
                 <div class="upload-dates col-sm-12" style="margin-bottom: 22px;">
                  ...
                  <button type="button" class="btn btncolour btnclass" onclick="getHtmlReport('html')">HTML REPORT</button></div> 
                   <div class="export">
                    ...
                    <div class="export">
                        <button id="clearnotdefault" type="button"  class="btn btncolour resettable btnclass"  ><i class="fa fa-share-square-o" aria-hidden="true"></i>CLEAR</button>
                    </div>
