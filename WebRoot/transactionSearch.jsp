...
<html lang="en">
  ...
  $( function() {
   ...
   $(document).ready(function(){
	   setHeader('transactionSearch.jsp');
   ...
   function getDocumentType(){
	   ...
	   var companyIdValue= '<%= cid %>';
		$.ajax({
			...
			success : function(data) {
			       for(i=0;i<data.response.length;i++){
			         $("#propertyType").append("<option value='"+data.response[i].transactionType+"'><b>"+data.response[i].transactionType+"</b></option>");
			      }  
			   },
			error : function(e) {
 ...
 function getTransactionStatus(){
   ...
   function clearResults(){
	...
	function getTransactionSearchList(type){
		$('#my_popup').popup('show');	
	    ...
	    /*  if($("#startDate").val() == ""){
	     ...
	     if($("#endDate").val() == ""){
		  ...
		  var testData={
				         "propertyId":$("#propertyId").val(),
				  	     ...
				  	     "propertyStatus":$("#transactionStatus").val(),
				  	     "companyId":companyId
		 ...
		 $.ajax({
		  type : "POST",
		  contentType : "application/json",
		  url : "<%=restURL%>/orcf/getTransactionSearchList",
	      dataType : 'json',
		  timeout : 100000,
		  data : JSON.stringify(testData),
		  success : function(data) {
			  $('#my_popup').popup('hide');
			  console.log("SUCCESS    : ", data.getTransactionSearchList.length);
				$("#propertyList").empty();
				$("#tableStyle1").show();
...
//     			$('.buttons-excel').css('margin-left', '136px');
	...
	function displayTransaction(data) {
		    ...
		    $.each(data.getTransactionSearchList, function(i, item) {
		    
			   item.transactionDate=='null'|| null ?item.transactionDate='':item.transactionDate == null ? item.transactionDate='':item.transactionDate;
			   item.transactionType =='null' || null ?item.transactionType='':item.transactionType ==null? item.transactionType='':item.transactionType;
		
			//$('#propertyList').append('<tr><td>'+item.transactionType+'</td><td><a id="editB" href="#" onclick="hello(\''+item.propertyId+'\',\''+item.propertyName+'\',\''+item.fhaNumber+'\',\''+item.loanType+'\');">'+item.propertyId+'</a></td><td>'+item.transactionDate+'</td></tr>');
		$('#propertyList').append('<tr><td><a id="editB" href="#" onclick="hello12(\''+item.propertyId+'\',\''+item.propertyName+'\',\''+item.fhaNumber+'\',\''+item.transactionType+'\',\''+item.transactionDate+'\',\''+item.transactionStatus+'\');">'+item.transactionType+'</td><td><a id="editB" href="#" onclick="hello(\''+item.propertyId+'\',\''+item.propertyName+'\',\''+item.fhaNumber+'\',\''+item.transactionType+'\',\''+item.transactionDate+'\',\''+item.transactionStatus+'\');">'+item.propertyId+'</a></td><td><a id="editB" href="#" onclick="hello123(\''+item.propertyId+'\',\''+item.propertyName+'\',\''+item.fhaNumber+'\',\''+item.transactionType+'\',\''+item.transactionDate+'\',\''+item.transactionStatus+'\');">'+item.transactionDate+'</td></tr>');
	...
	function hello(propertyId,propertyName,fhaNumber,transactionType,transactionDate,transactionStatus){
		//alert("item.fhaNumber"+fhaNumber)
	    ...
	    window.open("propertyDataSearch.jsp?propertyId="+propertyId+"&propertyName="+propertyName+"&fhaNumber="+fhaNumber+"&transactionType="+transactionType+"&transactionDate="+transactionDate+"&transactionStatus="+transactionStatus,"_blank",'PopUp'+randomnumber+',scrollbars=1,menubar=0,resizable=1,width=850,height=500');

	}

	function hello12(propertyId,propertyName,fhaNumber,transactionType,transactionDate,transactionStatus){
	//	alert("item.fhaNumber123"+fhaNumber)
		//	alert("transactionStatus"+transactionStatus)
		if(fhaNumber!=" ")
			{
			//alert("sdfafdaf")
		var res = propertyId + "/"+"Asset Management"+"/"+transactionType;
			}
		else
			{
			var res = propertyId + "/" +fhaNumber+ "/"+"Asset Management"+"/"+transactionType;
			
			}
	//	alert("hello2 else 323--"+propertyId+"======"+res);
		//alert("res"+res)
		 var randomnumber = Math.floor((Math.random()*100)+1); 
		    //window.open("propertyDataSearch.jsp?result="+res+"&propertyId="+propertyId+"&propertyName="+propertyName+"&fhaNumber="+fhaNumber+"&transactionType="+transactionType,"_blank",'PopUp'+randomnumber+',scrollbars=1,menubar=0,resizable=1,width=850,height=500');
	    window.open("propertyDataSearch.jsp?result="+res+"&propertyId="+propertyId+"&propertyName="+propertyName+"&fhaNumber="+fhaNumber+"&transactionType="+transactionType+"&transactionDate="+transactionDate+"&transactionStatus="+transactionStatus,"_blank",'PopUp'+randomnumber+',scrollbars=1,menubar=0,resizable=1,width=850,height=500');

	}
	function hello123(propertyId,propertyName,fhaNumber,transactionType,transactionDate,transactionStatus){
			//alert("transactionStatus"+transactionStatus)
		 var year = new Date(transactionDate).getFullYear();
		
		//alert("year"+year)
		
		if(fhaNumber!=" ")
			{
			var res =  "/" + propertyId + "/"+"Asset Management"+"/"+transactionType+"/"+year+"/"+transactionDate.split('/').join('-');+"/";

			}
		else{
		
		var res =  "/" + propertyId + "/" +fhaNumber+ "/"+"Asset Management"+"/"+transactionType+"/"+year+"/"+transactionDate.split('/').join('-');+"/";
	}
		//	alert("res"+res)
		 var randomnumber = Math.floor((Math.random()*100)+1); 
		// alert("hello2 else 358");
		    window.open("propertyDataSearch.jsp?result="+res+"&propertyId="+propertyId+"&propertyName="+propertyName+"&fhaNumber="+fhaNumber+"&transactionType="+transactionType+"&transactionDate="+transactionDate+"&transactionStatus="+transactionStatus,"_blank",'PopUp'+randomnumber+',scrollbars=1,menubar=0,resizable=1,width=850,height=500');
 ...
 */	    
   window.open(Text,"_blank",'PopUp'+randomnumber+',menubar=no,addressbar=no,resizable=no,scrollbars=yes,location=no,toolbar=no,width=950,height=800')
...
function getPdf(filename,fileSize) {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="col-lg-offset-4 col-lg-4 col-md-4 col-sm-12 col-xs-12">
							<div class="text-center">
								<h5><b class="f-family">Transaction Search</b></h4>
							</div>							
						</div>
							...
							<div class="pull-right">
								...
								<select id="selectArea" class="selectArea"  onchange="javascript:handleSelect(this.value)">
									...
									<option value="property" >Property Name Search</option>								
									 <option value="transaction" selected>Transaction Search</option> 				
								</select>
							</div>
						</div>
...
<!-- 												<option>Approved with Changes</option>								 -->
									...
									<!-- 	<div class="form-group row">
										...
										<!-- <div class="form-group row">
											...
											<div class="col-sm-8 posiRelavtive">
											  <select id="propertyStatus" class="selectArea width100per"> 
									...
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">		
										<br />
										<button class="btn btn-success waves-effect waves-button waves-float f-family center-block" onclick="javascript:getTransactionSearchList(this);"><i class="fa fa-check"></i>  Search</button>
					...
					<div class="">
						<div class="">
							<div class="row">
			                    ...
			                    <div class="table-responsive">
										<table id="searchResTableProp" class="table table-bordered">
									 	<thead>
												<tr>
													<th>Property Name</th>
													<th>Property ID</th>
